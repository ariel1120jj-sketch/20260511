...
function draw() {
  ...
  if (faces.length > 0 && faces[0].keypoints) {
    ...
    for (let i = 0; i < earPoints.length; i++) {
      let pt = earPoints[i];
      if (pt && earringImages[currentEarringIndex]) {
        // 座標映射
...
function drawMask(face, vWidth, vHeight) {
  if (!face || !face.keypoints || !maskImages[currentEarringIndex]) return;
  ...
  if (center && leftEdge && rightEdge) {
    ...
    let img = maskImages[currentEarringIndex];
    let mHeight = mWidth * (img.height / img.width);
